...
p_de_playa=1;
umbral_min = 0.025;  % Umbral mínimo para la detección de bordes
umbral_max = 0.121;  % Umbral máximo para la detección de bordes
else 
if(imagen_dir{1}(1)=='m')
umbral_min = 0.103234;  % Umbral mínimo para la detección de bordes
umbral_max = 0.248244;  % Umbral máximo para la detección de bordes
else 
if(imagen_dir{1}(1)=='C')
...
roca_evaluada=0;
% Variables booleanas relacionadas a las clases
arbol_asignado = false;
follaje_asignado = false;
pasto_asignado = false;
nube_asignada = false;
roca_asignada = false;
agua_asignada = false;
cielo_asignado = false;
arena_asignada = false;
lava_asignada = false;
for i = 1:size(Centros,1)
   ...
   % Comparar las distancias y asignar el nombre de la clase correspondiente
    ...
    elseif dist_azul_cielo_claro < 8
        array_clasificacion{i} = 'cielo';
        cielo_asignado = true;
    elseif dist_arena < 24 && p_de_playa == 1
        array_clasificacion{i} = 'arena';
        arena_asignada = true;
    elseif dist_lava < 13 && p_de_playa == 0
        array_clasificacion{i} = 'lava';
        lava_asignada = true;
    elseif dist_roca < 19 && roca_evaluada==0 
        array_clasificacion{i} = 'roca' ;
        roca_asignada = true;
        roca_evaluada=1;
    else
        array_clasificacion{i} = 'no class';
    end
end

% Reemplazar elementos repetidos por 'no class'
for i = 1:numel(array_clasificacion)
    if strcmp(array_clasificacion{i}, 'no class')
        continue;
    end
    for j = i+1:numel(array_clasificacion)
        if strcmp(array_clasificacion{i}, array_clasificacion{j})
            array_clasificacion{j} = 'no class';
        end
    end
end
...
for it=1:bucle
    if(array_clasificacion{it}~="cielo"&&(array_clasificacion{it}~="agua")&&(array_clasificacion{it}~="no class"))
...
tipo="Desconocido";
if(arena_asignada==1&&(agua_asignada==1))
tipo="playa";
elseif(pasto_asignado==1&&(cielo_asignado==1||nube_asignada)&&roca_asignada==0)
    tipo="pradera/llanura/campo";
elseif(roca_asignada==1&&(cielo_asignado==1||follaje_asignado==1)&&agua_asignada==0)
    tipo="montaña";
elseif(lava_asignada==1)
    tipo="volcan";
elseif(arbol_asignado==1)
    tipo="bosque";
end
title(strcat( "La imagen pertenece al tipo ",tipo));
