...
function imagenCorrespondencia(imgA, imgB)
    ...
    for i = 1:size(imgB, 1)
        for j = 1:size(imgB, 2)
            nivel_gris_actual = imgB(i, j, 3);
            idx_R = find(resultados_R(1, :) == nivel_gris_actual);
            idx_G = find(resultados_G(1, :) == nivel_gris_actual);
            idx_B = find(resultados_B(1, :) == nivel_gris_actual);
             if ~isempty(idx_R)
